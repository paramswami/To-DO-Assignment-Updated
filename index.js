...
function ToDoValidation(todoItem, varDate, category, status, varReminderDate) {
    ...
    if (varReminderDate != "") {
        var date_regex = /^((19|20)\d{2})\-(0[1-9]|1[0-2])\-(0[1-9]|1\d|2\d|3[01])$/;
        if (date_regex.test(varReminderDate)) {
            todoReq = true;
        }
        else {
            sptodo.innerHTML += " Please enter valid reminder date. "
            todoReq = false;
        }
    }

    if (varReminderDate == null || varReminderDate == "") {
        sptodo.innerHTML += " Please reminder enter date. "
        todoReq = false;
    } else if (sptodo.innerHTML == "") {
        sptodo.innerHTML = "";
    }
...
function BindTodo(reminder) {
    ...
    try {
        ...
        var txtReminder = document.getElementById("txtReminderSearch").value;
        ...
        if (sessionUserName == null || sessionUserName == "") {
            ...
            html1 += '<tr><th onclick="sortTable(0)" style="display:none";>to do id</th><th onclick="sortTable(1)">todoName</th><th onclick="sortTable(2)">Category</th><th onclick="sortTable(3)">Status</th><th onclick="sortTable(4)">Date</th><th onclick="sortTable(5)">ReminderDate</th><th>Action</th></tr>';
                    ...
                    for (let j = 0; j < registerObj[i][0].todo.length; j++) {
                        ...
                        if (status != "" && category != "") {
                            if (registerObj[i][0].todo[j].status == status && registerObj[i][0].todo[j].category == category) {
                                html1 += '<tr> <td  style="display:none"; >' + count + '</td><td>' + registerObj[i][0].todo[j].todoName + '</td><td>' + registerObj[i][0].todo[j].category + '</td><td>' + registerObj[i][0].todo[j].status + '</td><td>' + registerObj[i][0].todo[j].varDate + '</td><td>' + registerObj[i][0].todo[j].varReminderDate + '</td><td><button type="submit" onclick="GetToDoFunc(' + j + ', \'' + registerObj[i][0].todo[j].todoName + '\' , \'' + registerObj[i][0].todo[j].category + '\' , \'' + registerObj[i][0].todo[j].status + '\' , \'' + registerObj[i][0].todo[j].varDate + '\', \'' + registerObj[i][0].todo[j].varReminderDate + '\')" >Edit</button> &nbsp;&nbsp; <button type="submit" onclick="DeleteToDoFunc(' + j + ')" >Delete</button></td> </tr>';
                            }
                        }
                        else if (status != "") {
                            ...
                            if (txtReminder != "") {
                                if (registerObj[i][0].todo[j].varReminderDate > txtReminder) {
                                    html1 += '<tr> <td  style="display:none"; >' + count + '</td><td>' + registerObj[i][0].todo[j].todoName + '</td><td>' + registerObj[i][0].todo[j].category + '</td><td>' + registerObj[i][0].todo[j].status + '</td><td>' + registerObj[i][0].todo[j].varDate + '</td><td>' + registerObj[i][0].todo[j].varReminderDate + '</td><td><button type="submit" onclick="GetToDoFunc(' + j + ', \'' + registerObj[i][0].todo[j].todoName + '\' , \'' + registerObj[i][0].todo[j].category + '\' , \'' + registerObj[i][0].todo[j].status + '\' , \'' + registerObj[i][0].todo[j].varDate + '\', \'' + registerObj[i][0].todo[j].varReminderDate + '\')" >Edit</button> &nbsp;&nbsp; <button type="submit" onclick="DeleteToDoFunc(' + j + ')" >Delete</button></td> </tr>';
                                }
                            }
                        }
...
function ToDoFunc(btnVal) {
    try {
        ...
        let varReminderDate = document.getElementById("txtReminderDate").value;
        ...
        todoReq = ToDoValidation(todoItem, varDate, category, status, varReminderDate);
                ...
                if (registerObj[i][0].email == sessionUserName) {
                    ...
                    if (btnVal == "add")  //Add New Todo Item
                    {
                        ...
                        var newItem = {
                            ...
                            todoName: todoItem,
                            varDate: varDate,
                            varReminderDate: varReminderDate,
                            category: category,
                            status: status
                        };
                    ...
                    else if (btnVal == "edit")  //Edit Todo Item
                    {
                        ...
                        registerTaskJson[i][0].todo[index].varReminderDate = varReminderDate;
...
function GetToDoFunc(index, todoValue, category, Status, vardate, varReminderDate) {
    ...
    document.getElementById("txtReminderDate").value = varReminderDate;
...
function CleartodoFilter() {
    ...
    document.getElementById("select_Reminder").selectedIndex = 0;
    document.getElementById("txtReminderSearch").value = "";
    document.getElementById("txtReminderSearch").style.display = "none";
    document.getElementById("btnReminder").style.display = "none";
    BindTodo();
}
...
function checkReminder() {
    ...
    var dateReminder = document.getElementById("select_Reminder").value;

    if (dateReminder == "Yes") {
        document.getElementById("txtReminderSearch").style.display = "";
        document.getElementById("btnReminder").style.display = "";
    }
    else {
        document.getElementById("txtReminderSearch").style.display = "none";
        document.getElementById("btnReminder").style.display = "none";
